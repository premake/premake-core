...
--
	...
	function project.eachconfig(prj, field, filename)
		local configpairs = project.getconfigpairs(prj)
		
		local i = 0		
		return function ()
			i = i + 1
			if i <= #configpairs then
				local pairing = configpairs[i]
...
--
-- Returns a list of build configuration/platform pairs for a project,
-- with any configuration mappings applied.
--

	function project.getconfigpairs(prj)
		local platforms = prj.platforms or {}

		-- pair up the build cfgs and platforms specified in the project
		local result = {}

		for _, buildcfg in ipairs(prj.configurations or {}) do
			if prj.platforms and #prj.platforms > 0 then
				for _, platform in ipairs(prj.platforms) do
					table.insert(result, { buildcfg, platform })
				end
			else
				table.insert(result, { buildcfg })
			end
		end
		
		-- apply any config mappings
		for patterns, replacements in pairs(prj.configmaps or {}) do
			if type(patterns) ~= "table" then
				patterns = { patterns }
			end
			
			for i, pairing in ipairs(result) do
				result[i] = project.applyconfigmap(pairing, patterns, replacements)
			end
		end
			
		-- remove any duplicates generated by the mapping
		
		return result	
	end

	function project.applyconfigmap(pairing, patterns, replacements)
		-- compare the pattern against the pairing; the whole pattern
		-- (which may be one or two parts) must match the same number
		-- of items in the pairing
		for i = 1, #pairing do
			local matched = true
			for j = 1, #patterns do
			...
			if matched then
				if #patterns == 1 and #replacements == 1 then
					...
					return { replacements[1], replacements[2] }
				end	
			end
		end
