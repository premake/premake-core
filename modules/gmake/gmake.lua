...
--
	...
	function gmake.copyfile_cmds(source, dest)
	...
	function gmake.mkdirRules(dirname)
		_p('%s:', dirname)
...
--
	...
	function gmake.list(value, quoted)
		quoted = false
		if #value > 0 then
			if quoted then
				local result = ""
...
--
	...
	function gmake.tovar(value)
		value = value:gsub("[ -]", "_")
		value = value:gsub("[()]", "")
		return value
	end

	function gmake.getToolSet(cfg)
	...
	function gmake.outputSection(prj, callback)
		local root = {}
...
---------------------------------------------------------------------------
	...
	function gmake.phonyRules(prj)
		_p('.PHONY: clean prebuild')
		_p('')
	end


	function gmake.shellType()
	...
	function gmake.buildCmds(cfg, event)
	...
	function gmake.preBuildCmds(cfg, toolset)
		gmake.buildCmds(cfg, "prebuild")
	end


	function gmake.preLinkCmds(cfg, toolset)
		gmake.buildCmds(cfg, "prelink")
	end


	function gmake.postBuildCmds(cfg, toolset)
		gmake.buildCmds(cfg, "postbuild")
	end


	function gmake.targetDirRules(cfg, toolset)
		gmake.mkdirRules("$(TARGETDIR)")
	end


	function gmake.objDirRules(cfg, toolset)
		gmake.mkdirRules("$(OBJDIR)")
	end


	function gmake.preBuildRules(cfg, toolset)
		_p('prebuild: | $(OBJDIR)')
		_p('\t$(PREBUILDCMDS)')
		_p('')
	end
